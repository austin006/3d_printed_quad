...
def main():
    ...
    node.declare_parameter('vehicle_id', '')
    vehicle_id = node.get_parameter('vehicle_id').get_parameter_value().string_value
    ...
    if vehicle_id:
        node.get_logger().info(f"--- Teleop controlling Vehicle ID: {vehicle_id} ---")
        namespace = f"/px4_{vehicle_id}"
    else:
        node.get_logger().info("--- Teleop running in default single-vehicle mode ---")
        namespace = ""

    # --- Create publishers with dynamic topic names ---
    vel_cmd_topic = f"{namespace}/offboard_velocity_cmd" if namespace else "/offboard_velocity_cmd"
    arm_cmd_topic = f"{namespace}/arm_message" if namespace else "/arm_message"

    node.get_logger().info(f"Publishing velocity to: {vel_cmd_topic}")
    node.get_logger().info(f"Publishing arm command to: {arm_cmd_topic}")

    pub = node.create_publisher(geometry_msgs.msg.Twist, vel_cmd_topic, qos_profile)
    arm_toggle = False
    arm_pub = node.create_publisher(std_msgs.msg.Bool, arm_cmd_topic, qos_profile)
